...
var Zepto = (function () {
  ...
  $.extend = function (target) {
    // target 参数
    var deep,
      args = slice.call(arguments, 1); // 提取从第二个参数开始的值 2，3
    if (typeof target == "boolean") { // target取第一个值  是否深度合并
      deep = target; // deep 深拷贝
      target = args.shift(); // 取提取后的第一个参数 剩余源值
    }
    args.forEach(function (arg) { //  遍历源
      extend(target, arg, deep); // 拷贝 传参 布尔值，目标，是否深拷贝
    });
    return target;
  };
  ...
  $.fn = {
    ...
    is: function (selector) {
      return typeof selector == "string" ?
        this.length > 0 && zepto.matches(this[0], selector) :
